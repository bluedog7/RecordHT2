...
int main(int argc, char * argv[])
{
	...
	if (strcasecmp(extname, "mp4"))
	{
		printf("%s is not mp4 file\r\n", filename);
		print_help();
		return -1;
	}

	MP4CTX * p_ctx = mp4_read_open(filename);
	if (NULL == p_ctx)
	{
		printf("mp4_read_open (%s) failed\r\n", filename);
		return -1;
	}
